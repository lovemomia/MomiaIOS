...
- (void)requestData:(BOOL)refresh {
    ...
    self.curOperation = [[HttpService defaultService]GET:URL_APPEND_PATH(@"/product/leader")
                                              parameters:paramDic cacheType:CacheTypeDisable JSONModelClass:[FavouriteModel class]
                                                 success:^(AFHTTPRequestOperation *operation, id responseObject) {
                                                     ...
                                                     if (orderListModel.data.totalCount == 0) {
                                                         [self.view showEmptyView:@"没有找到可以领队的活动，敬请期待哦~"];
                                                         return;
                                                     }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if(indexPath.row < self.list.count) {
        ...
        NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"duola://selectsku?pid=%ld", (long)product.pID]];
