...
RCT_EXPORT_METHOD(uploadAudioFile:(RCTResponseSenderBlock)callback) {
    ...
    NSString *databasePath = [[NSString alloc] initWithString: [docsDir stringByAppendingPathComponent:@"test.aac"]];
    
    NSLog(@"%@",databasePath);
    
    [[HttpService defaultService] uploadAudioWithFilePath:databasePath fileName:@"audioFile.aac" handler:^(NSURLResponse *response, id responseObject, NSError *error) {
